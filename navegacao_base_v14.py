#!/usr/bin/env python3
"""
BIBLIOTECA DE NAVEGACAO v13 - EV3 / ev3dev2
...
v8  - causa raiz isolada e corrigida empiricamente:
...
v9  -Seguidor de linha funcionando, limpeza geral do codigo, comeco da garra e braco.

v10 -braco e garra implementados, junto com os comandos.

v11 -programacao de primeira missao.

v12 -troca do sistema de rodas: motor de tracao CONTINUA sendo o
    LargeMotor (grande), um por lado - nao mudou. O que mudou e a
    lateral em si: agora tem 3 engrenagens identicas de 4cm (uma em
    cada eixo de roda + uma louca no meio, sincronizando as duas
    rodas do mesmo lado) no lugar da roda com pneu 58x28 antiga.
    Relacao motor->roda continua 1:1 (engrenagens iguais, engrenadas
    direto), entao motor_esq.position segue valendo como graus da
    roda sem fator de conversao extra.
    - motor_class NAO mudou (continua LargeMotor em robo/motor_esq/
      motor_dir) - confirmado com a equipe que o motor de movimento
      e o mesmo de antes, so a mecanica da roda mudou.
    - DIAMETRO_RODA atualizado de 5.8 (pneu 58x28 antigo) para o novo
      diametro de contato com o chao. VALOR ABAIXO E PROVISORIO -
      medir com fita metrica / andar_cm(50) como fizemos na v3/v4 e
      ajustar antes de confiar em qualquer distancia.
    - PENDENTE (nao mudado neste commit, precisa bancada):
      * BIAS_RETO: o desvio mecanico documentado na v3 foi medido com
        a roda antiga. A engrenagem louca no meio muda o atrito/folga
        de cada lado, entao o desvio pode ter mudado de valor mesmo
        com o motor igual. Remedir do jeito de sempre
        (seguir_linha_por_cm em trecho reto).
      * VEL_LINHA / VEL_ANCORAGEM negativos (bug v5-v8): como o motor
        continua o mesmo (LargeMotor), a principio esse workaround
        continua valendo - nao seria motivo pra retestar por causa da
        troca de roda. So vale reconferir se notarem o robo andando
        de re em algum trecho novo.

v13 -bug corrigido: girar_para() (usado em gangorra()/viagem_exemplo())
    ficava girando infinitamente. Causa: angulo() nao estava mais
    invertendo o sinal do giroscopio - so fazia "return giro.angle"
    puro, contrariando a propria nota da v3 ("angulo() criada: inverte
    o sinal do giroscopio, que conta ao contrario do steering do
    ev3dev2 neste robo"). Sem a inversao, o erro (angulo_alvo -
    angulo()) empurra o robo pro lado ERRADO a cada iteracao do loop
    de girar_para, entao o erro nunca cai abaixo de TOLERANCIA_ANGULO
    e o while True nunca sai - giro infinito. Provavelmente uma
    regressao de uma edicao anterior (a funcao tinha uma indentacao
    sobrando, sinal de codigo apagado ali no meio).
    - CORRIGIDO: angulo() agora faz "return -giro.angle" de novo.
    - Adicionado tempo_max (padrao 5s) em girar_para()/girar_relativo()
...
PORTA_SENSOR_ESQ = INPUT_1
PORTA_SENSOR_DIR = INPUT_2
PORTA_GIRO = INPUT_4
PORTA_GARRA = OUTPUT_C     # motor medio - abre/fecha as garras
PORTA_BRACO = OUTPUT_B     # motor grande - sobe/desce o braco
PORTA_ULTRASSONICO = INPUT_3

motor_garra = MediumMotor(PORTA_GARRA)
motor_braco = LargeMotor(PORTA_BRACO)
...
botao = Button()
sensor_prox = UltrasonicSensor(PORTA_ULTRASSONICO)
...
# ==================== VELOCIDADES ====================
# VEL_LINHA e VEL_ANCORAGEM negativos = workaround do bug v5-v8 (motor
# grande invertendo em velocidade baixa). Motor de tracao nao mudou na
# v12, entao isso continua valendo - so reconferir se o robo comecar a
# andar de re em algum trecho.
VEL_NAVEGACAO = 45
VEL_LINHA = -15
VEL_GIRO = 15
VEL_ANCORAGEM = -10
...
# TODO v12: valor medido com a roda/pneu antigos - a engrenagem louca
# nova muda o atrito de cada lado, remedir do zero antes de confiar.
BIAS_RETO = 3
...
# ==================== GIROSCOPIO ====================
def angulo():
    """Inverte o sinal do giroscopio (ele conta ao contrario do steering
    do ev3dev2 neste robo - ver nota da v3 no cabecalho). Todas as
    funcoes de giro devem usar angulo(), nunca giro.angle direto."""
    return -giro.angle
...
def girar_para(angulo_alvo, velocidade=VEL_GIRO, tempo_max=5.0):
    ...
    while True:
        ...
        if abs(erro) <= TOLERANCIA_ANGULO or botao.any():
            break
        if time() - inicio > tempo_max:
            print("AVISO: girar_para nao convergiu em", tempo_max,
                  "s (alvo:", angulo_alvo, "| atual:", angulo(),
                  ") - confira o sinal do giroscopio (angulo()).")
            break
...
#=====================MOVIMENTO GARRA====================
def abrir_garra(graus=GRAUS_GARRA, velocidade=VEL_GARRA):
    """Gira o motor da garra para abrir."""
    motor_garra.on_for_degrees(velocidade, graus, brake=True)


def fechar_garra(graus=GRAUS_GARRA, velocidade=VEL_GARRA):
    """Gira no sentido contrario para fechar/segurar o objeto."""
    motor_garra.on_for_degrees(-velocidade, graus, brake=True)


def levantar_braco(graus=GRAUS_BRACO, velocidade=VEL_BRACO):
    motor_braco.on_for_degrees(-velocidade, graus, brake=True)


def abaixar_braco(graus=GRAUS_BRACO, velocidade=VEL_BRACO):
    """Comando contrario ao levantar_braco - mesmo curso, sentido invertido."""
    motor_braco.on_for_degrees(velocidade, graus, brake=True)
...
#===========================MEDIÇÃO============================
def medir_cm():
    return sensor_prox.distance_centimeters
